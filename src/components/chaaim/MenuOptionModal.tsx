...
export default function MenuOptionModal({ open, item, onClose, onConfirm }: Props) {
  ...
  const addonPrice = useMemo(() => {
    ...
    for (const opt of item.options) {
      if (opt.id === "note") continue; // กันกรณีพิเศษ
      ...
      <div style={styles.modal} onClick={(e) => e.stopPropagation()}>
        ...
        <div style={styles.header}>
          <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
            {/* ป้องกัน image ว่าง */}
            {imgSrc ? (
              <img src={imgSrc} alt={item.name} style={styles.thumb} />
            ) : (
              <div
                style={{
                  ...styles.thumb,
            ...
            .map((opt) => (
              <div key={opt.id} style={{ marginBottom: 12 }}>
                <div style={{ fontWeight: 600, marginBottom: 6 }}>
                  {opt.label}
                  {opt.required ? <span style={{ color: "#e22" }}> *</span> : null}
                </div>

                <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                  {opt.choices.map((c: Choice) => {
                    const picked = (selected[opt.id] ?? []).includes(c.id);
                    const priceText = c.price ? ` +${fmtTHB(c.price)}` : "";
        ...
        <div style={styles.footer}>
          ...
          <button onClick={confirm} style={styles.addBtn}>
            เพิ่มลงตะกร้า • {fmtTHB(total)}
          </button>
        </div>
      </div>
    </div>
